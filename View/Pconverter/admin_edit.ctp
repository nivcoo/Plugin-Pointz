        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header with-border">
                    <h3 class="card-title"><?= $Lang->get('POINTZ__MODIFY_OFFER') ?></h3>
                </div>
                <div class="card-body">
                    <form action="" method="post" data-ajax="true" data-redirect-url="<?= $this->Html->url(array('controller' => 'pconverter', 'action' => 'index', 'admin' => true)) ?>">

                        <div class="ajax-msg"></div>
                        <input type="hidden" name="id" value="<?= $getItemsConverter['PointzItemsConverter']['id'] ?>">

                        <div class="form-group">
                            <label><?= $Lang->get('POINTZ__OFFER_NAME') ?></label>
                            <input name="name" value="<?= $getItemsConverter['PointzItemsConverter']['name'] ?>" class="form-control" type="text">
                        </div>

                        <div class="form-group">
                            <label><?= $Lang->get('POINTZ__ITEM_ICON') ?></label>
                            <input name="icon" value="<?= $getItemsConverter['PointzItemsConverter']['icon'] ?>" class="form-control" type="text">
                        </div>

                        <div class="form-group">
                            <label><?= $Lang->get('POINTZ__OFFER_PRICE') ?></label>
                            <input name="price" value="<?= $getItemsConverter['PointzItemsConverter']['price'] ?>" class="form-control" type="text">
                        </div>

                        <div class="form-group">
                            <label><?= $Lang->get('POINTZ__ITEM_PRICEIG') ?></label>
                            <input name="price_ig" value="<?= $getItemsConverter['PointzItemsConverter']['price_ig'] ?>" class="form-control" type="text">
                        </div>
                                ...
                                <?php
                                    ...
                                    foreach ($lores as $key => $value) {
                                        ...
                                        echo '<span class="input-group-append">';
                                            echo '<button class="btn btn-danger delete-lore" data-id2="'.$j.'" type="button"><span class="fa fa-times"></span></button>';
                                        echo '</span>';
                                        echo '</div>';
                                    }
                                ?>
                            ...
                            <div class="addCommand">
                                <?php
                                    ...
                                    foreach ($commands as $key => $value) {
                                        ...
                                        echo '<span class="input-group-append">';
                                            echo '<button class="btn btn-danger delete-cmd" data-id="'.$i.'" type="button"><span class="fa fa-times"></span></button>';
                                        echo '</span>';
                                        echo '</div>';
                                    }
                                ?>
...
<script>
    $('#addCommand').on('click', function(e) {
        ...
        input += '<button class="btn btn-danger delete-cmd" data-id="' + i + '" type="button"><span class="fa fa-times"></span></button>';
        input += '</span>';
        input + '</div>';
    ...
    $('#addLore').on('click', function(e) {
        ...
        input += '<span class="input-group-append">';
        input += '<button class="btn btn-danger delete-lore" data-id2="' + j + '" type="button"><span class="fa fa-times"></span></button>';
        input += '</span>';
        input + '</div>';
